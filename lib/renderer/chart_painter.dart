...
class ChartPainter extends BaseChartPainter {
  ...
  void initChartRenderer() {
    mMainRenderer = MainRenderer(mMainRect, mMainMaxValue, mMainMinValue,
        this.chartStyle.topPadding, mainState, isLine, fixedLength, this.chartStyle,
      this.chartColors, this.scaleX,);
    if (mVolRect != null) {
      mVolRenderer ??= VolRenderer(mVolRect!, mVolMaxValue, mVolMinValue,
          this.chartStyle.childPadding, fixedLength, this.chartStyle, this.chartColors);
    }
  ...
  // final Paint mBgPaint = Paint()..color = this.chartColors.bgColor;
  ...
  void drawBg(Canvas canvas, Size size) {
    ...
    Rect mainRect = Rect.fromLTRB(
        0, 0, mMainRect.width, mMainRect.height + this.chartStyle.topPadding);
    canvas.drawRect(mainRect, mBgPaint..shader = mBgGradient.createShader(mainRect));

    if (mVolRect != null) {
      Rect volRect = Rect.fromLTRB(0, mVolRect!.top - this.chartStyle.childPadding,
          mVolRect!.width, mVolRect!.bottom);
      canvas.drawRect(volRect, mBgPaint..shader = mBgGradient.createShader(mainRect));
    }

    if (mSecondaryRect != null) {
      Rect secondaryRect = Rect.fromLTRB(
          0,
          mSecondaryRect!.top - this.chartStyle.childPadding,
          mSecondaryRect!.width,
          mSecondaryRect!.bottom);
      canvas.drawRect(secondaryRect, mBgPaint..shader = mBgGradient.createShader(mainRect));
    }
    Rect dateRect = Rect.fromLTRB(
        0, size.height - this.chartStyle.bottomDateHigh, size.width, size.height);
    canvas.drawRect(dateRect, mBgPaint..shader = mBgGradient.createShader(mainRect));
  }

  @override
  void drawGrid(canvas) {
    mMainRenderer.drawGrid(
        canvas, this.chartStyle.gridRows, this.chartStyle.gridColumns);
    mVolRenderer?.drawGrid(canvas, this.chartStyle.gridRows, this.chartStyle.gridColumns);
  ...
  void drawRightText(canvas) {
    var textStyle = getTextStyle(this.chartColors.yAxisTextColor);
    mMainRenderer.drawRightText(canvas, textStyle, this.chartStyle.gridRows);
    mVolRenderer?.drawRightText(canvas, textStyle, this.chartStyle.gridRows);
    mSecondaryRenderer?.drawRightText(canvas, textStyle, this.chartStyle.gridRows);
  }

  @override
  void drawDate(Canvas canvas, Size size) {
    double columnSpace = size.width / this.chartStyle.gridColumns;
    ...
    for (var i = 0; i <= this.chartStyle.gridColumns; ++i) {
      ...
      if (translateX >= startX && translateX <= stopX) {
        ...
        TextPainter tp = getTextPainter(getDate(datas[index].id!),
            color: this.chartColors.xAxisTextColor);
        y = size.height -
            (this.chartStyle.bottomDateHigh - tp.height) / 2 -
            tp.height;
  ...
  //   ..isAntiAlias = true
  //   ..strokeWidth = 0.5
  //   ..color = this.chartColors.markerBgColor;
  // Paint selectorBorderPaint = Paint()
  //   ..isAntiAlias = true
  //   ..strokeWidth = 0.5
  //   ..style = PaintingStyle.stroke
  //   ..color = this.chartColors.markerBorderColor;
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    y = size.height - this.chartStyle.bottomDateHigh;
  ...
  void drawMaxAndMin(Canvas canvas) {
    ...
    if (x < mWidth / 2) {
      ...
      TextPainter tp = getTextPainter("── ${format(mMainLowMinValue)}",
          color: this.chartColors.maxMinTextColor);
  ...
  void drawCrossLine(Canvas canvas, Size size) {
    ...
    double y = getMainY(point.close);
    // k线图竖线
    canvas.drawLine(Offset(x, this.chartStyle.topPadding),
        Offset(x, size.height - this.chartStyle.bottomDateHigh), paintY);
...
//    canvas.drawCircle(Offset(x, y), 2.0, paintX);
  ...
  void drawRealTimePrice(Canvas canvas, Size size) {
    ...
    TextPainter tp = getTextPainter(format(point.close),
        color: this.chartColors.rightRealTimeTextColor);
    double y = getMainY(point.close);
    //max越往右边滑值越小
    ...
    if (tp.width < max) {
      while (startX < max) {
        canvas.drawLine(
            ...
            realTimePaint..color = this.chartColors.realTimeLineColor);
        startX += space;
      }
      //画一闪一闪
      if (isLine) {
      ...
      canvas.drawRect(
          ...
          realTimePaint..color = this.chartColors.realTimeBgColor);
      ...
      while (startX < mWidth) {
        canvas.drawLine(Offset(startX, y), Offset(startX + dashWidth, y),
            realTimePaint..color = this.chartColors.realTimeLongLineColor);
        startX += space;
      }
      ...
      double left =
          mWidth - mWidth / this.chartStyle.gridColumns - tp.width / 2 - padding * 2;
      ...
      canvas.drawRRect(
          rectBg2, realTimePaint..color = this.chartColors.realTimeTextBorderColor);
      canvas.drawRRect(
          rectBg1, realTimePaint..color = this.chartColors.realTimeBgColor);
      tp = getTextPainter(format(point.close),
          color: this.chartColors.realTimeTextColor);
